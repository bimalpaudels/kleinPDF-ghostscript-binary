    ...
    def progress_hook(block_num, block_size, total_size):
        if total_size > 0:
            ...
            if block_num % 50 == 0 or percent >= 100:  # Update every ~50 blocks or at completion
                print(f"    Progress: {percent:.1f}% ({block_num * block_size // 1024 // 1024}MB)", end='\r')
    ...
    with tarfile.open(tar_path, 'r:gz') as tar:
        ...
        for i, member in enumerate(members):
            ...
            if i % 100 == 0 or i == total_files - 1:  # Update every 100 files
                ...
                print(f"    Extracting: {percent:.1f}% ({i + 1}/{total_files} files)", end='\r')
        
        print()  # New line after progress
    log_progress("✅ Extraction completed", start_time)

def setup_ram_disk(size_mb=2048):
    """Set up RAM disk for faster compilation (macOS/Linux)"""
    try:
        if sys.platform == "darwin":  # macOS
            # Create RAM disk
            sectors = size_mb * 2048  # 512-byte sectors
            result = subprocess.run(['hdiutil', 'attach', '-nomount', f'ram://{sectors}'], 
            ...
            subprocess.run(['mount', '-t', 'hfs', device, str(ram_disk_path)], check=True)
...
def run_command_with_progress(cmd, cwd=None, check=True, description=None):
    ...
    try:
        ...
        if isinstance(cmd, list) and cmd[0] == 'make':
            process = subprocess.Popen(cmd, shell=False, cwd=cwd, 
            ...
            while True:
                ...
                if output:
                    ...
                    elapsed = time.time() - start_time
                    if (len(output_lines) % 20 == 0 and elapsed > 10) or \
                       any(keyword in output.lower() for keyword in ['compiling', 'linking', 'building']):
                        print(f"    Build progress: {elapsed:.0f}s elapsed, {len(output_lines)} operations completed")
            
            result_code = process.poll()
            if result_code is None:
                result_code = 0
            if result_code != 0 and check:
                raise subprocess.CalledProcessError(result_code, cmd, '\n'.join(output_lines[-20:]))
            
            log_progress(f"✅ {desc}", start_time)
            return type('Result', (), {'returncode': result_code, 'stdout': '\n'.join(output_lines)})()
        else:
...
def build_ghostscript(cleanup=True, use_ram_disk=True):
    ...
    if use_ram_disk:
        ram_disk_path, ram_device = setup_ram_disk(2048)  # 2GB RAM disk
    ...
    try:
        with tempfile.TemporaryDirectory(dir=work_base) as temp_dir:
            ...
            log_progress(f"📁 Working in: {temp_path} {'(RAM disk)' if ram_disk_path else '(regular disk)'}")
            ...
            for i, configure_cmd in enumerate(configure_attempts):
                result = run_command_with_progress(configure_cmd, cwd=gs_source_dir, check=False,
            ...
            make_jobs = min(cpu_count + 2, 12)  # Cap at 12 to avoid overwhelming the system
            
            # Don't override CFLAGS in make - they're already set in configure
            make_cmd = ["make", f"-j{make_jobs}"]
            
            run_command_with_progress(make_cmd, cwd=gs_source_dir,
                                    description=f"Building Ghostscript ({make_jobs} parallel jobs)")
            
            # Install to build directory
            run_command_with_progress(["make", "install"], cwd=gs_source_dir,
            ...
            if gs_binary.exists():
                ...
                log_progress(f"🎉 Standalone Ghostscript binary created: {final_binary.absolute()}")
                
                # Test the binary
                log_progress("🧪 Testing binary functionality")
                result = run_command_with_progress([str(final_binary), "--version"], check=False,
                ...
                if result.returncode == 0:
                    ...
                    if cleanup:
                        ...
                        if cleaned_files:
                            log_progress(f"🗑️  Removed test files: {', '.join(cleaned_files)}")
                        
                        log_progress("✅ Cleanup completed!")
                    else:
                        log_progress("📁 Build artifacts retained (use --cleanup to remove)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build standalone Ghostscript binary with performance optimizations")
    ...
    parser.add_argument("--no-ram-disk", action="store_true",
